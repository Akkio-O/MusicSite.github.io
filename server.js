...
expressWs(app);
app.use(express.json()); // для разборка json тела запроса
app.use(cors());
...
app.post('/reg', function (req, res) {
    ...
    // Проверка на валидность пароля (например, длина)
    ...
    con.query(checkEmailQuery, [email], function (err, result) {
        ...
        con.query(insertUserQuery, [firstName, lastName, phone, email, password], function (err, result) {
            if (err) {
                console.error("Ошибка при выполнении запроса:", err);
                return res.status(500).json({ error: 'Произошла ошибка при обработке запроса' });
            }
            // Получаем данные о только что зарегистрированном пользователе
            const getUserQuery = "SELECT first_name, email FROM users WHERE id = ?";
            const userId = result.insertId;
            con.query(getUserQuery, [userId], function (err, userResult) {
                if (err) {
                    console.error("Ошибка при выполнении запроса:", err);
                    return res.status(500).json({ error: 'Произошла ошибка при обработке запроса' });
                }
...
// Маршрут для обработки запросов с параметрами
...
app.get(['/category', '/category/products/:IC_GROUP0?/:IC_GROUP1?/:productName?', '/category/products/:IC_GROUP0?/:IC_GROUP1?/:IC_GROUP2?/:productName?'], async function (req, res) {
    try {
        ...
        // Проверяем маршрут с базой данных
